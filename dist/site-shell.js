...
(function(){"use strict";async function r(e){const i=await fetch(e);if(!i.ok)throw new Error("Failed to load site config");return i.json()}function o(){return window.location.pathname}function l(e,i,t){return e.menu.map(s=>{const a=t+s.path,n=i.startsWith(s.path);return`
      <a href="${a}" class="${n?"active":""}">
        ${s.label}
      </a>
    `}).join(""),`
  ...
  `}class d extends HTMLElement{async connectedCallback(){const i=this.getAttribute("config")||"config/site-config.json",t=this.getAttribute("base")||"",s=await r(i+"?v="+Date.now()),a=o(),n=this.innerHTML;this.innerHTML=`
      ${l(s,a,t)}
